...
export default function ScheduleGrid({
  ...
  return (
    <div className="bg-[#0a0a0a] rounded-none border border-white/10 p-3 sm:p-5 flex flex-col h-full space-y-4 sm:space-y-5" id="schedule-grid-container">
      ...
      <div className="flex flex-col xl:flex-row gap-4 items-start xl:items-center justify-between border-b border-white/10 pb-4 shrink-0">
        <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3 w-full sm:w-auto">
          <div className="flex bg-white/[0.03] border border-white/10 p-0.5 rounded-none w-full sm:w-auto">
            <button
              onClick={() => setViewMode("grid")}
              className={`flex-1 sm:flex-none px-3 py-2 rounded-none flex items-center justify-center space-x-1.5 transition-all text-[9.5px] sm:text-[10px] font-bold uppercase tracking-wider font-mono cursor-pointer ${
          ...
          <div className="relative w-full sm:w-48">
            ...
            <input
              ...
              className="pl-9 pr-4 py-2 text-[10.5px] border border-white/10 rounded-none bg-white/[0.03] w-full focus:outline-none focus:border-white transition-all font-mono tracking-wide text-white"
            />
          </div>
        </div>
        ...
        <div className="flex items-center gap-2 w-full xl:w-auto font-sans">
          <button
            ...
            className="flex-1 sm:flex-none py-2 px-3 border border-white/20 bg-white/5 hover:bg-white hover:text-black text-white text-[9px] sm:text-[10px] font-bold uppercase tracking-widest font-mono rounded-none flex items-center justify-center space-x-1.5 transition-all cursor-pointer h-9"
          >
          ...
          <button
            onClick={exportToCSV}
            className="flex-1 sm:flex-none py-2 px-3 bg-white hover:bg-slate-200 text-black text-[9px] sm:text-[10px] font-bold uppercase tracking-widest font-mono rounded-none flex items-center justify-center space-x-1.5 transition-all outline-none cursor-pointer h-9"
          >
      ...
      <div className="grid grid-cols-1 xs:grid-cols-2 lg:grid-cols-4 gap-3 bg-white/[0.02]/40 p-3 sm:p-4 rounded-none border border-white/10">
        {/* Department Track */}
        <div className="space-y-1">
          ...
          <select
            ...
            className="w-full bg-[#050505] border border-white/10 rounded-none p-2 text-[10.5px] font-sans font-medium text-white outline-none focus:border-white/45 cursor-pointer appearance-none"
          >
        ...
        {/* Study Level Track */}
        <div className="space-y-1">
          <label className="block text-[8px] uppercase font-bold text-white/40 tracking-widest font-mono">Level</label>
          <select
            value={selectedLevel}
        ...
        <div className="space-y-1">
          <label className="block text-[8px] uppercase font-bold text-white/40 tracking-widest font-mono">Location</label>
          <select
            value={selectedVenue}
          ...
          <table className="min-w-[850px] w-full table-fixed border-collapse select-none sm:select-text touch-pan-x">
            <thead>
              <tr className="bg-white/[0.02]/90 border-b border-white/10 text-white/50 text-[10px] sm:text-xs text-left">
                ...
                {DAYS.map((day) => (
                  <th key={day} className="px-2 sm:px-4 py-3 text-center font-bold font-mono tracking-widest text-[9px] sm:text-[10px] uppercase text-white border-l border-white/10">
                    {day.substring(0, 3)}
                  </th>
                ))}
              </tr>
                          ...
                          <div className="space-y-2">
                            {cellAllocations.map((alloc) => {
                              ...
                              return (
                                <div
                                  key={alloc.id}
                                  className={`p-2 sm:p-3 rounded-none border text-[10px] sm:text-xs relative transition-all group flex flex-col justify-between h-24 sm:h-28 shadow-xs ${getDeptColor(crs.department)} ${
